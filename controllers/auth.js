...
const register = async (req, res, next) => {
  ...
  try {
    const mailOptions = {
      ...
      templateOptions: {
        confirmationLink: `${process.env.APP_URL}/auth/register/confirm/${result.token.value}`
      }
    };
...
const forgotPassword = async (req, res, next) => {
  ...
  try {
    const mailOptions = {
      ...
      userId: user.id,
      templateOptions: {
        resetLink: `${process.env.APP_URL}/auth/password/reset/${passwordResetToken.value}`
      }
    };
...
const resetPassword = async (req, res, next) => {
  ...
  const token = await Token.findOne({ where: { value: resetPasswordToken } });
  ...
  await user.save();
  await Token.destroy({ where: { value: resetPasswordToken } });
